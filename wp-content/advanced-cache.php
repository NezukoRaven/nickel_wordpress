...
// WP-Optimize advanced-cache.php (written by version: 3.7.0) (homeurl: http://nickel.local/) (abspath: C:\Users\Nezuko Raven Games\Local Sites\nickel\app\public/) (do not change this line, it is used for correctness checks)
...
$possible_plugin_locations = array(
	...
	'C:\Users\Nezuko Raven Games\Local Sites\nickel\app\public\wp-content\plugins\wp-optimize\cache',
);
...
if (false === $plugin_location) {
	...
	if (strcasecmp('http://nickel.local/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', 'C:\Users\Nezuko Raven Games\Local Sites\nickel\app\public\wp-content\plugins\wp-optimize\cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-nickel.local.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-nickel.local.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-nickel.local.php');
}
